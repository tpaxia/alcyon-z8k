...
FILE	*newfile();
	...
	if((sp = strrchr(srcfile,'/')) == 0) sp = srcfile; else sp++;
	if(strlen(sp) > 14) sp[14] = '\0';
	strcpy(titl1,srcfile);
	strcpy(fname,sp);
	if((ep = strrchr(fname,'.')) == 0)
		usage();
	else
		ep++;
	if( segflg ) {		/* Prog name must end with "8ks" or "s". */
	    if( strcmp (ep,"8ks") != 0 && strcmp(ep,"s") != 0) {
		printf("Segmented source file must end with '.8ks' or '.s'\n");
		usage();
	    }
	}
	else 			/* Prog name must end in "8kn" or "s". */
	    if( strcmp (ep,"8kn") != 0 && strcmp(ep,"s") != 0) {
		printf("Nonsegmented source file must end with '.8kn' or '.s'\n");
		usage();
	    }
...
usage() {
	...
	fprintf(ERROR,"Usage:  asz8k [-o outfile] [-luxs] file.{8kn|8ks|s}\n");
	exit(1);
}
