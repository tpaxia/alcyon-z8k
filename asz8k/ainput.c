...
iovck() {
	...
	if(insp > (char *)instk+INSIZ*sizeof(int)) {
...
popin() {
	...
	curlst = infp->in_lst&0377;  insp = (char *)infp;  infp = infp->in_ofp;
}
...
struct	input	*newfp;
	...
	while((uintptr_t)insp % sizeof(void *)) insp++;  /* force pointer alignment */
	newfp = (struct input *)insp;
