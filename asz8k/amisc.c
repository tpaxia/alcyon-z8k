...
/*
 * agetline - Reads the next line of source, and puts it into the global
 ...
 * and returns a pointer to the block.  Uses a pool allocator to keep
 * allocations contiguous (required for format table entries).
 */
#define POOLSIZ	(256*1024)
char *
palloc(size) uns size; {

char	*p;

	if(phytop == 0 || phytop + size > phylim) {
		phytop = malloc(POOLSIZ);
		if(phytop == 0) {
			fprintf(ERROR,"Out of memory\n");
			exit(1);
		}
		phylim = phytop + POOLSIZ;
	}
	p = phytop;
	phytop += size;
	return(p);
