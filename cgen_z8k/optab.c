...
const char *const mnemonics[] = {
	...
	"sub",		/* I_SUB */
	"dec",		/* I_DEC */
	"mult",		/* I_MULS (signed multiply) */
	"multu",	/* I_MULU (unsigned multiply) */
	"div",		/* I_DIVS (signed divide) */
	"divu",		/* I_DIVU (unsigned divide) */
	"sra",		/* I_SRA (shift right arithmetic) */
	"srl",		/* I_SRL (shift right logical) */
	"sla",		/* I_SLA (shift left arithmetic) */
	"sll",		/* I_SLL (shift left logical) */
	"and",		/* I_AND */
	"or",		/* I_OR */
	"xor",		/* I_XOR */
	"neg",		/* I_NEG */
	"com",		/* I_COM (complement, was "not" on 68000) */
	"ld",		/* I_LD (was "move") */
	"clr",		/* I_CLR */
	"cp",		/* I_CP (compare, was "cmp") */
	"tst",		/* I_TST */
	"lmul",		/* I_LMUL (library: long multiply) */
	"_ldiv",	/* I_LDIV (library: long divide) */
	"lrem",		/* I_LREM (library: long remainder) */
	"almul",	/* I_LEML (library) */
	"aldiv",	/* unused 26 */
	"alrem",	/* I_LERM (library) */
	"jr eq,",	/* I_JR_EQ */
	"jr ne,",	/* I_JR_NE */
	"jr gt,",	/* I_JR_GT (signed) */
	"jr ge,",	/* I_JR_GE (signed) */
	"jr lt,",	/* I_JR_LT (signed) */
	"jr le,",	/* I_JR_LE (signed) */
	"jr ule,",	/* I_JR_ULE (unsigned <=) */
	"jr ult,",	/* I_JR_ULT (unsigned <) */
	"jr uge,",	/* I_JR_UGE (unsigned >=) */
	"jr ugt,",	/* I_JR_UGT (unsigned >) */
	"jp ",		/* I_JP_T (unconditional jump) */
	"*nop",		/* I_NOP */
	"bit",		/* I_BIT */
};
 ...
 *
 * IDENTICAL structure to 68000 — only mnemonic indices differ.
 */
char const optab[][6] = {
...
/*    I       I2	  effect	  cc's    stack   register */
	...
	{ I_SRA,  I_SRL,  I_NULL,     FC_FIX, I_NULL, FR_SHFT },	/* 6=RSH */
	{ I_SLA,  I_SLL,  I_NULL,     FC_FIX, I_NULL, FR_SHFT },	/* 7=LSH */
	{ I_AND,  I_AND,  I_NULL,     FC_FIX, FS_OP,  FR_ADD },	/* 8=AND */
	{ I_OR,   I_OR,   I_NULL,     FC_FIX, FS_OP,  FR_ADD },	/* 9=OR */
	{ I_XOR,  I_XOR,  I_NULL,     FC_FIX, I_NULL, FR_XOR },	/* 10=XOR */
	{ I_NULL, I_NULL, I_NULL,     FC_FIX, I_NULL, I_NULL },	/* 11=NOT */
	{ I_NEG,  I_NEG,  I_NULL,     FC_FIX, I_NULL, FR_NEG },	/* 12=NEG */
	{ I_COM,  I_COM,  I_NULL,     I_NULL, I_NULL, FR_NEG },	/* 13=COMPL */
	...
	{ I_LD,   I_CLR,  FE_ASSIGN,  I_NULL, I_NULL, FR_ASSIGN },/* 18=ASSIGN */
	...
	{ I_SRA,  I_SRL,  FE_EQSHFT,  I_NULL, I_NULL, FR_EQSHFT },/* 24=EQRSH */
	{ I_SLA,  I_SLL,  FE_EQSHFT,  I_NULL, I_NULL, FR_EQSHFT },/* 25=EQLSH */
	{ I_AND,  I_AND,  FE_EQOP,    I_NULL, I_NULL, FR_EQOP },	/* 26=EQAND */
	{ I_OR,   I_OR,   FE_EQOP,    I_NULL, I_NULL, FR_EQOP },	/* 27=EQOR */
	{ I_XOR,  I_XOR,  FE_EQXOR,   FC_FIX, I_NULL, FR_EQXOR },/* 28=EQXOR */
	{ I_NULL, I_NULL, I_NULL,     I_NULL, I_NULL, FR_CALL },	/* 29=FJSR */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 30=EQUALS */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 31=NEQUALS */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 32=GREAT */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 33=GREATEQ */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 34=LESS */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, I_NULL, I_NULL },	/* 35=LESSEQ */
	{ I_NULL, I_NULL, I_NULL,     I_NULL, FS_ITL, FR_ITL },	/* 36=INT2L */
	{ I_NULL, I_NULL, I_NULL,     I_NULL, I_NULL, FR_LTI },	/* 37=LONG2I */
	{ I_BIT,  I_BIT,  I_NULL,     FC_BTST,I_NULL, I_NULL },	/* 38=BTST */
	{ I_CP,   I_TST,  I_NULL,     FC_REL, FS_LD,  FR_LD },	/* 39=LOAD */
	...
	{ I_COM,  I_COM,  FE_EQNOT,   I_NULL, I_NULL, FR_EQNOT },/* 47=EQNOT */
...
/*
 * Z8002 branch table.
 * Maps comparison operators × signedness to branch instructions.
 * Column 0 = signed, column 1 = unsigned.
 */
char const brtab[][2] = {
	{ I_JR_EQ,  I_JR_EQ },		/* EQUALS */
	{ I_JR_NE,  I_JR_NE },		/* NEQUALS */
	{ I_JR_GT,  I_JR_UGT },	/* GREAT */
	{ I_JR_GE,  I_JR_UGE },	/* GREATEQ */
	{ I_JR_LT,  I_JR_ULT },	/* LESS */
	{ I_JR_LE,  I_JR_ULE }		/* LESSEQ */
};
...
/*
 * Code skeleton built-in strings for Z8002.
 * These are indexed by (token - MOV) in expand().
 */
const char *const strtab[] = {
	...
	"call",		/* JSR */
	"clr",		/* CLR */
	"clr",		/* CLRL - Z8002 clr works for all sizes via suffix */
	"extsb",	/* EXTW - byte→word sign extension */
	"exts",		/* EXTL - word→long sign extension */
	"lda",		/* LEA - load effective address */
	"@R15",		/* STK - stack reference */
};
