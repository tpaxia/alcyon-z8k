...
PP(const struct skeleton *skp;)				/* pointer to code skeleton */
{
	...
	short pop_pending;			/* Z8002: bytes to pop after current instruction */
	short skel_bol;				/* Z8002: beginning-of-line state for tab indentation */
	...
	while ((c = *macro++) != 0)
	{
		...
		default:
			...
			oputchar(c);
			/*
			 * Z8002: after emitting a newline, flush pending stack adjustments.
			 * This handles the case where POP was used as a source operand
			 * (e.g., "add Rn,@R15\n") — we need "inc R15,#N" to actually
			 * pop the value since Z8002 has no auto-increment addressing.
			 */
			if (c == '\n')
			{
				skel_bol = 1;
		...
		case POP:
			/*
			 * Z8002: POP is used as a source operand in skeletons like
			 * "OP CR,POP" meaning "operate on top-of-stack into register".
			 * We output @R15 as the memory operand and schedule a stack
			 * adjustment after the instruction (at the next newline).
			 * Pop size depends on the expression type.
			 */
			{
				short popsz = LONGTYPE(tp->t_type) ? (INTSIZE * 2) : INTSIZE;
				stacksize--;
				oprintf("@R15");
				pop_pending += popsz;
			}
		...
		case PSH:
			/*
			 * Z8002: PSH is a PREFIX in skeletons. It emits the push
			 * instruction and destination, then the following skeleton bytes
			 * provide the source operand. Pattern: PSH <source> '\n'
			 *
			 * FORSTACK: push @R15,<src> (pre-decrement SP, then store)
			 * FORSP:    ld @R15,<src>   (store at current SP, no move)
			 *
			 * For long types, uses pushl/ldl instead of push/ld.
			 */
			if (skel_bol) { oputchar('\t'); skel_bol = 0; }
			{
				short ptype = LEAFOP(op) ? tp->t_type : ltp->t_type;
				if (cookie == FORSP)
				{
					if (LONGTYPE(ptype))
						oprintf("ldl @R15,");
		...
		case LEA:
		case STK:
			if (skel_bol && c != STK) { oputchar('\t'); skel_bol = 0; }
			oprintf("%s", strtab[c - MOV]);
			break;

		case EXTW:
			/* extsb Rn — always uses word register name, skip ' ' CR '\n' */
			if (skel_bol) { oputchar('\t'); skel_bol = 0; }
			oprintf("extsb R%d", freg);
			if (*macro == ' ') macro++;
			if ((unsigned char)*macro == CR) macro++;
			break;

		case EXTL:
			/* exts RRn — always uses register pair name, skip ' ' CR '\n' */
			if (skel_bol) { oputchar('\t'); skel_bol = 0; }
			oprintf("exts RR%d", freg & ~1);
			if (*macro == ' ') macro++;
			if ((unsigned char)*macro == CR) macro++;
			break;
		...
		case CR:
			outcreg(freg, tp->t_type);
			break;

		case NR:
			outcreg(nreg, tp->t_type);
			break;

		case CAR:
			outcreg(AREG(freg), INT);
			break;

		case NAR:
			outcreg(AREG(nreg), INT);
			break;
		...
		case MODSWAP:
			/*
			 * Z8002: DIV RRn,src puts quotient in R(n+1) and remainder in R(n).
			 * For DIV: need to move quotient from R(n+1) to R(n) = CR.
			 * For MOD: remainder already in R(n) = CR, no move needed.
			 */
			if (skel_bol) { oputchar('\t'); skel_bol = 0; }
			switch (op)
			{
			...
			case MOD:
			case EQMOD:
				/* remainder already in CR = R(freg), no move needed */
				break;
			default:
				/* DIV/LDIV/EQDIV: move quotient from R(freg+1) to CR */
				oprintf("ld R%d,R%d\n", freg, freg + 1);
				break;
			}
			break;
