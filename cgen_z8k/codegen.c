...
PP(int reg;)
{
	...
	{
		/*
		 * Z8002: No cc save/restore instructions available.
		 * Force ccflag 2 → 1 (retest via outcmp0) since
		 * OUTCCSAVE/OUTCCRESTORE are no-ops.
		 */
		if (ccflag == 2)
			ccflag = 1;
 ...
 *
 * Z8002: Use LDIR @Rdst,@Rsrc,Rcount for block transfer.
 * LDIR copies count words from @Rsrc to @Rdst, incrementing both.
 */
...
PP(int size;)								/* structure size to copy */
{
	...
	if (cflag)
		oprintf("size is %d\n", size);
#endif
	count = (size + 1) / INTSIZE;  /* word count */
	if (count <= 0)
		return;
	oprintf("\tld R%d,#%d\n", DREG(lr), count);
	oprintf("\tldir @R%d,@R%d,R%d\n", lr, rr, DREG(lr));
}
...
PP(register int reg;)						/* first available register */
{
	...
	case CALL:
	case NACALL:
		/* Z8K: emit .global for external function references so
		   asz8k generates relocation entries for the linker */
		if (tp->t_left->t_op == SYMBOL &&
			tp->t_left->t_sc == EXTERNAL)
			oprintf("\t.global _%.*s\n", SSIZE,
				tp->t_left->t_symbol);
		ssize = 0;
		savestk = stacksize;
